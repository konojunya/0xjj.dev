...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      const pointer = pointers.at(0);
      if (!pointer || touches.length === 0) return;
      for (let i = 0; i < touches.length; i++) {
        const posX = scaleByPixelRatio(touches[i]?.clientX ?? 0 );
        const posY = scaleByPixelRatio(touches[i]?.clientY ?? 0);
        updateFrame();
        updatePointerDownData(pointer, touches[i]?.identifier ?? 0, posX, posY);
      }
