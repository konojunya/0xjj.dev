...
import { createInitialState, placeDisc, resetForRematch } from "./logic";
import type { ClientMessage, GameState, Mark, ServerMessage } from "./types";

const RECONNECT_TIMEOUT = 60_000;

export class ReversiRoom extends DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (!mark) {
      if (!this.state.players.B) mark = "B";
      else if (!this.state.players.W) mark = "W";
    ...
    if (this.state.players.B?.connected && this.state.players.W?.connected && this.state.status === "waiting") {
    ...
    const opp = this.getWs(mark === "B" ? "W" : "B");
  ...
  async webSocketMessage(ws: WebSocket, rawMessage: string | ArrayBuffer): Promise<void> {
    ...
    switch (msg.type) {
      case "place": {
        ...
        this.state = placeDisc(this.state, msg.row, msg.col, mark);
      ...
      case "rematch": {
        ...
        if (this.state.rematchRequested.B && this.state.rematchRequested.W) this.state = resetForRematch(this.state);
        await this.saveState();
        this.broadcastState();
        break;
      }
  ...
  async alarm(): Promise<void> {
    ...
    if (changed) {
      if (!this.state.players.B && !this.state.players.W) {
        this.state = createInitialState();
        this.playerIds = {};
      }
      await this.saveState();
