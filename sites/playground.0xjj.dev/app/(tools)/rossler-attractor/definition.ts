...
void main() {
  ...
  // View
  float zoom = mix(0.025, 0.05, zoomN);
  ...
  // Trail length
  int steps = int(mix(300.0, 600.0, trailN));
  float fSteps = float(steps);

  // Initial condition near attractor
  vec3 pos = vec3(1.0, 1.0, 1.0);
  vec3 ctr = vec3(0.0, 0.0, 3.0);

  // Warmup — advance start based on time for flowing animation
  int warmup = 200 + int(mod(u_time * mix(4.0, 16.0, speedN), 400.0));
  for (int i = 0; i < 600; i++) {
    if (i >= warmup) break;
    vec3 d = vec3(-pos.y - pos.z, pos.x + a * pos.y, b + pos.z * (pos.x - c));
    pos += d * dt;
  }
